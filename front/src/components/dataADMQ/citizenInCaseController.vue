<template>
    <div>
        ...
        <div>
            <!-- 输入框中默认提示为警局id -->
            <input type="text" v-model="caseID" placeholder="案件编号" />
            ...
            <input type="text" v-model="IDNum" placeholder="涉案人员身份证号" />
        ...
        <table v-if="caseInfo.length > 0">
            ...
            <tbody>
                <tr v-for="item of caseInfo" :key="`${item.caseID}-${item.idNum}`">
                    <td>{{ item.caseID }}</td>
                    <td>{{ item.caseType }}</td>
                    <td>{{ item.status }}</td>
...
export default {
    ...
    methods: {
        fetchCaseInfo() {
            axios.post('http://localhost:7078/api/citizenInCaseInfo', {
                caseID: this.caseID,
